...
def train_and_validate(args):
    ...
    # 初始化优化器：adaptive 模式下把可学习损失的参数作为独立参数组（weight_decay=0）
    param_groups = [{'params': model.parameters(), 'weight_decay': 1e-4}]
    if criterion is not None and args.loss_mode == 'adaptive':
        param_groups.append({'params': criterion.parameters(), 'weight_decay': 0.0})
        print("将可学习损失参数加入优化器（weight_decay=0）")
    ...
    # 如果指定了恢复点，则加载检查点（含可选的 criterion/optimizer 状态）
    if args.resume is not None:
        model, start_epoch, best_f1, cur_iter = load_checkpoint(
            args.resume, model, criterion, optimizer
        )
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if (epoch + 1) % args.val_interval == 0:
            ...
            if current_f1 > best_f1:
                ...
                torch.save(checkpoint_dict, join(save_path, 'checkpoint.pth.tar'))

...
@torch.no_grad()
def validate_model(args, val_loader, model, epoch, criterion=None):
    """
    在验证集上验证模型性能。
    ...
    Args:
        ...
        model: 待验证的模型
        epoch: 当前epoch数
        criterion: 可选的复合损失模块，为None时回退到BCEDiceLoss
    ...
    for iter_idx, batched_inputs in enumerate(val_loader):
        ...
        # 计算损失：有复合损失用复合损失，否则回退BCEDiceLoss
...
def train_epoch(args, train_loader, model, optimizer, epoch, max_batches,
                cur_iter=0, lr_factor=1., criterion=None):
    """
    训练模型一个epoch。
    ...
    for iter_idx, batched_inputs in enumerate(train_loader):
        ...
        # 计算损失：有复合损失用复合损失，否则回退BCEDiceLoss
        if criterion is not None:
            loss = criterion(output_mask, target)
        else:
            loss = BCEDiceLoss(output_mask, target)
...
def train_and_validate_single_config(args, ablation_config_id, exp_dir):
    ...
    # 构造复合损失：loss_mode 为 none 时回退到 BCEDiceLoss（不创建 criterion）
    criterion = None
    if args.loss_mode != 'none':
        criterion = CompositeBCEIoUFocalLoss(
            mode=args.loss_mode,
            initial_weight=args.loss_weight,
        ).cuda()
        print(f"使用复合损失: mode={args.loss_mode}, initial_weight={args.loss_weight}")
    else:
        print(f"使用 BCEDiceLoss（loss_mode=none）")    
    # 微调功能：加载预训练模型
    ...
    # 加载检查点（含可学习损失状态；optimizer 在构造后单独恢复）
    if checkpoint_to_load is not None:
        try:
            print(f"正在加载检查点: {checkpoint_to_load}")
            model, start_epoch, best_f1, cur_iter = load_checkpoint(
                checkpoint_to_load, model, criterion, optimizer=None
            )
            # 记录检查点路径，供 optimizer 构造后恢复
            _pending_optimizer_ckpt = checkpoint_to_load
    ...
    if args.finetune is not None:
        ...
        args.max_epochs = original_max_epochs // 2
        print(f"微调模式：将轮数从 {original_max_epochs} 调整为 {args.max_epochs}")

    # 折算总迭代数：训练口径为固定epoch数（默认100），步数随数据集规模自动折算
    args.max_steps = args.max_epochs * max_batches
    print(f"每个epoch {max_batches} 个批次，共 {args.max_epochs} 个epoch，折算总迭代数: {args.max_steps}")
    ...
    # adaptive 模式下把可学习损失的参数作为独立参数组
    # weight_decay=0 避免对 theta 施加 L2 正则；lr_scale=0.5 让 weight_logit
    # 以主学习率的一半更新，减缓 w 单调漂向 1.0 的精度坍塌趋势
    param_groups = [{'params': model.parameters(), 'weight_decay': 1e-4}]
    if criterion is not None and args.loss_mode == 'adaptive':
        param_groups.append({
            'params': criterion.parameters(),
            'weight_decay': 0.0,
            'lr_scale': 0.5,
        })
        print("将可学习损失参数加入优化器（weight_decay=0, lr_scale=0.5）")
    ...
    # 恢复优化器状态（如有待恢复的检查点）
    if _pending_optimizer_ckpt is not None:
        try:
            ckpt = torch.load(_pending_optimizer_ckpt, weights_only=False)
            optimizer.load_state_dict(ckpt['optimizer'])
            print("✅ 已恢复优化器状态")
        except (KeyError, ValueError) as e:
            print(f"⚠️  优化器状态恢复失败（不影响训练）: {e}")
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if (epoch + 1) % args.val_interval == 0:
            torch.cuda.empty_cache()
            loss_val, score_val = validate_model(
                args, val_loader, model, epoch, criterion=criterion
            )
            ...
            if current_f1 > best_f1:
                ...
                torch.save(checkpoint_dict, join(exp_dir, 'checkpoint.pth.tar'))
    ...
    if os.path.exists(join(exp_dir, 'best_model.pth')):
        ...
        # 在测试集上评估
        loss_test, score_test = validate_model(
            args, test_loader, model, 0, criterion=criterion
        )
        test_log_str = (
...
def run_ablation_experiment(args):
    ...
    config_names = {
        ...
        7: "X3D + Attention + ASPP + Transformer",
...
def run_all_ablation_experiments(args):
    ...
    print("支持的配置:")
    print("  配置1: X3D")
    print("  配置2: X3D + Attention")
    print("  配置3: X3D + ASPP")
    print("  配置4: X3D + Transformer")
    print("  配置5: X3D + Attention + ASPP")
    print("  配置6: X3D + ASPP + Transformer")
    print("  配置7: X3D + Attention + ASPP + Transformer")
    print("  配置8: X3D + Attention + Transformer")
    ...
    if args.configs is not None:
        ...
        configs_to_run = [1, 2, 3, 4, 5, 6, 7, 8]  # 默认运行八组配置
        print("运行所有配置")
...
def run_finetune_experiments(args):
    ...
    if args.finetune_configs is None:
        configs_to_finetune = [1, 2, 3, 4, 5, 6, 7, 8]  # 默认微调八组配置
    else:
    ...
    for config_id in configs_to_finetune:
        if config_id not in [1, 2, 3, 4, 5, 6, 7, 8]:
...
def get_parser():
    ...
    parser.add_argument(
        ...
        help='损失模式：none=BCEDiceLoss，fixed=固定权重复合损失，adaptive=可学习权重复合损失（默认）。',
    )
    parser.add_argument(
        '--loss_weight',
        type=float,
        default=0.3,
        help='复合损失中Focal项的初始/固定权重w（仅loss_mode为fixed/adaptive时生效）。',
    )
...
def main():
    ...
    # 检查配置ID是否有效
    if args.ablation_config is not None and args.ablation_config not in [1, 2, 3, 4, 5, 6, 7, 8]:
        parser.error(f"不支持的配置ID: {args.ablation_config}，支持的范围: 1-8")

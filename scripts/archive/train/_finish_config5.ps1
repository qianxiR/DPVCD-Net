...
Write-Output ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] Resume config_5 (X3D+A+B) from checkpoint...")
...
$p.WaitForExit()
$code = $p.ExitCode
Write-Output ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] config_5 EXIT code=" + $code + " PID=" + $p.Id)
Write-Output ("config_5 DONE. Stopping. config_6 and config_7 will NOT be started.")

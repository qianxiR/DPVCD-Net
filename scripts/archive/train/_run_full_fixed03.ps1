...
Write-Output ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] START config_7 (Full) fixed w=0.3")
...
$p.WaitForExit()
$code = $p.ExitCode
Write-Output ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] config_7 (Full fixed w=0.3) EXIT code=" + $code)

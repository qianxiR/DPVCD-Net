...
用于加载已训练的模型并在测试集上进行评估

使用示例：
# 测试单个模型 - GVLM-CD数据集
python scripts/test/test_BCD.py --file_root "E:\rqx\dataes\GVLM-CD" --model_path ./exp_new/GVLM-CD/best_model.pth --batch_size 8

# 测试单个模型 - WHU-CD数据集
python scripts/test/test_BCD.py --file_root "E:\rqx\dataes\WHU-CD" --model_path ./exp_new/WHU-CD/best_model.pth --batch_size 8

# 测试单个模型 - LBFD-CD数据集
python scripts/test/test_BCD.py --file_root "E:\rqx\dataes\LBFD-CD" --model_path ./exp_new/LBFD-CD/best_model.pth --batch_size 8

# 测试并保存预测结果
python scripts/test/test_BCD.py --file_root "E:\rqx\dataes\WHU-CD" --model_path ./exp_new/WHU-CD/best_model.pth --save_predictions --output_dir ./predictions

# 详细模式（显示每个batch的结果）
python scripts/test/test_BCD.py --file_root "G:\deeplearning\DAVCD-Net实验数据\LBFD-CD" --model_path ./exp_new/LBFD-CD/best_model.pth --verbose
"""
...
def save_predictions(args, predictions, targets, scores):
    ...
    output_dir = join(args.output_dir, dataset_name)
...
def load_model(args):
    ...
    if not missing_keys and not unexpected_keys:
        ...
        print(f"   模拟前向传播成功初始化了所有动态模块")
    elif missing_keys and not unexpected_keys:
        print(f"⚠️  警告: 模型有 {len(missing_keys)} 个参数未从文件加载")
        print(f"   这些参数将使用随机初始化，测试结果可能不准确")
    elif unexpected_keys and not missing_keys:
        print(f"✅ 模型所有必需参数已成功加载")
        if len(unexpected_keys) > 0:
            print(f"ℹ️  权重文件包含 {len(unexpected_keys)} 个额外参数（已被忽略）")
    else:
        print(f"⚠️  警告: 模型与权重文件部分不匹配")
        print(f"   缺少: {len(missing_keys)} 个 | 多余: {len(unexpected_keys)} 个")
    
    # 验证关键模块
    print(f"\n🔍 关键模块验证:")
    if hasattr(model, 'encoder'):
        print(f"   ✓ Encoder (X3D backbone)")
    if hasattr(model, 'pos_encoder') and hasattr(model.pos_encoder, 'edge_projection'):
        print(f"   ✓ Position Encoder ({len(model.pos_encoder.edge_projection)} 个尺度)")
    if hasattr(model, 'attention_modules'):
        print(f"   ✓ Attention Modules ({len(model.attention_modules)} 个尺度)")
    if hasattr(model, 'aspp_modules'):
        print(f"   ✓ ASPP Modules ({len(model.aspp_modules)} 个尺度)")
    if hasattr(model, 'st_fusion'):
        print(f"   ✓ ST Fusion (Hierarchical Cross Attention)")
...
def get_parser():
    ...
    - parser: 配置好的参数解析器
    """
    parser = ArgumentParser(description='变化检测模型测试脚本')

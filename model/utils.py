...
def adjust_learning_rate(args, optimizer, epoch=None, iter=None, max_batches=None, 
    ...
    if args.lr_mode == 'step':
        ...
        lr = args.learning_rate * (0.1 ** (epoch // args.step_loss))
        ...
        lr = args.learning_rate * (1 - cur_iter * 1.0 / max_iter) ** 0.9
    ...
    if epoch == 0 and iter is not None and iter < 200:
        lr = args.learning_rate * 0.9 * (iter + 1) / 200 + 0.1 * args.learning_rate
    
    # Apply additional lr factor
    lr *= lr_factor

    # Update learning rate for all parameter groups
    # 尊重每个参数组的 lr_scale，使可学习损失参数等能以独立比例衰减
    for param_group in optimizer.param_groups:
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # assume inputs are probabilities after sigmoid
        eps = 1e-6
        inputs = torch.clamp(inputs, eps, 1.0 - eps)
        pt = inputs * targets + (1 - inputs) * (1 - targets)  # pt = p if y == 1 else 1-p
        loss = -self.alpha * (1 - pt) ** self.gamma * torch.log(pt)
        return loss.mean()

def BCEDiceFocalLoss(inputs, targets, focal_weight=0.3):
    """
    inputs: sigmoid output, shape (B, 1, H, W)
    ...
    dice_score = (2 * intersection + smooth) / (inputs.sum(dim=(1,2,3)) + targets.sum(dim=(1,2,3)) + smooth)
    ...
    # Combine
    combined = (1 - focal_weight) * (bce + dice_loss) + focal_weight * focal_loss
    return combined


def load_checkpoint(resume_path, model, criterion=None, optimizer=None):
    """
    Load a checkpoint from a given path.

    入参:
    - resume_path (str): 检查点文件路径
    - model (nn.Module): 待加载 state_dict 的模型
    - criterion (nn.Module, optional): 可选，待加载 criterion_state_dict 的损失模块
    - optimizer (torch.optim.Optimizer, optional): 可选，待加载优化器状态的优化器

    方法:
    - 读取检查点元数据 (epoch/best_f1/cur_iter)
    - 以 strict=False 加载模型权重，兼容结构不一致（消融配置切换）
    - criterion/optimizer 仅在传入且检查点存在对应字段时加载，否则跳过

    出参:
    - tuple: (model, start_epoch, best_f1, cur_iter)
    """
    start_epoch = 0
    best_f1 = 0.
    cur_iter = 0
    ...
    if resume_path is not None and os.path.isfile(resume_path):
        ...
        cur_iter = checkpoint.get('cur_iter', 0)

        # Load model state with strict=False to handle configuration mismatches
        ...
        # Find missing keys in current model
        ...
        if unexpected_keys:
            print(f"Warning: {len(unexpected_keys)} unexpected keys in checkpoint (likely from different config):")
        ...
        if criterion is not None and 'criterion_state_dict' in checkpoint:
            ...
            print(f"=> loaded criterion_state_dict")
        ...
        print(f"=> loaded checkpoint '{resume_path}' (epoch {start_epoch}, best_f1 {best_f1:.4f})")
        print(f"=> successfully loaded {len(filtered_state_dict)}/{len(state_dict)} parameters")
    else:
        print(f"=> no checkpoint found at '{resume_path}' or path is not valid.")

    return model, start_epoch, best_f1, cur_iter


def setup_logger(log_filename, save_path, args=None):
    """
    Set up logging and optionally log all command line arguments.
    
    Args:
        log_filename (str): Name of the log file.
        save_path (str): Path to the save directory.
        args (optional): Command line arguments object to log.
        
    Returns:
        Logger file handle.
    """
    log_file_loc = join(save_path, log_filename)
    logger = open(log_file_loc, 'a+', encoding='utf-8')
    
    # Log all arguments if provided
    if args:
...
def caption_accuracy(scores, targets, k):
    """
    Computes top-k accuracy for captions.
    """
